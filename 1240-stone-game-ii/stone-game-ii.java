class Solution {
    ...
    public int helper(int[] piles, int idx, int person, int m, int[][][] dp){
        if(idx >= n){
            return 0;
        }
        ...
        for(int i = 1; i<=Math.min(2*m,n-idx); i++){
            stone += piles[idx+i-1];
            if(person == 0){
                res = Math.max(res,stone+helper(piles,idx+i,1,Math.max(m,i),dp));
            }
            else{
                res = Math.min(res,helper(piles,idx+i,0,Math.max(m,i),dp));
            }
        }
